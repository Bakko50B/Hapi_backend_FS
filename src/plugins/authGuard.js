...
export default async function authGuard(request, h) {
  try {
    ...
    // Försök läsa token från cookie
    if (request.state?.token) {
      token = request.state.token;
    }

    //  Om ingen cookie → försök läsa Authorization-header
    if (!token) {
    ...
    // Om fortfarande inget token → inte inloggad
    if (!token) {
